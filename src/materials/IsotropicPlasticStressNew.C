#include "IsotropicPlasticStressNew.h"
#include "MooseEnum.h"
#include "MooseTensor.h"

registerMooseObject("LynxApp", IsotropicPlasticStressNew);

InputParameters
IsotropicPlasticStressNew::validParams()
{
...
IsotropicPlasticStressNew::IsotropicPlasticStressNew(
...
void
IsotropicPlasticStressNew::initQpStatefulProperties()
{
...
void 
IsotropicPlasticStressNew::computeQpPK1Stress()
{
  ...
  // Elastic jacobian
  _pk1_jacobian[_qp] = _bulk_modulus * IdentityTwoTwo + 2. * _shear_modulus * IdentityFourDev;
  ...
  if(stress_eq > yield_stress.value)
  {
    ...
    while(true)
    {
      ...
      if (abs(r) < _tolerance * yield_stress.value)
        ...
        mooseException("IsotropicPlasticStressNew: Plasticity not converging");
    ...
    _pk1_jacobian[_qp] += 6. * _shear_modulus * _shear_modulus * (
  ...
  _pk1_stress[_qp] = stress_vol * Identity + stress_dev;
}

void 
IsotropicPlasticStressNew::computeLameConstants()
{
...
FunctionValue
IsotropicPlasticStressNew::getYieldStress(Real plastic_strain_eq)
{
